...
墨墨梧文（Mò Mò Wú Wén）是一款专为投标文件场景设计的 Web 排版工具，目标群体为招投标人员、商务专员和标书制作团队。

**灵感来源**：中国古典水墨卷轴 × 现代杂志编辑风格。视觉上以羊皮纸底色、朱红强调、金色点缀、水墨书法字体传递东方审美，交互上采用三栏式专业布局（标签导航 + 预览区 + 设置面板），对标 Adobe InDesign 的编辑体验。

**核心价值**：投标文件排版具有高度重复性——每份标书都需按招标文件要求设置页边距、标题字体、正文行距、图表编号规则等参数。墨墨梧文将这一过程从手动操作（Word 逐项设置）变为"上传 → 打标签 → 一键生成"的三步流水线，将排版时间从小时级压缩到分钟级。
...
### 在线编辑
- 基于 `@eigenpal/docx-editor-vue` 的所见即所得编辑器
- 支持修订模式、评论标注
- 切换开关：编辑模式 ↔ 只读预览模式

### 排版标签系统

7 个标签面板覆盖投标文件排版的全部维度：

| 标签 | 功能 |
|------|------|
| **页面** | 纸张尺寸（A4/A3/A5 等）、页边距（上下左右）、装订线、页眉边距、分栏数（1/2/3）、分栏间距 |
| **正文** | 中文字体/英文字体/字号、粗斜体下划线、行距模式与值、段前段后间距、左右缩进、首行缩进、对齐方式、段间距设置 |
| **标题** | 4 级标题各自的字体/字号/粗斜体/行距/段前段后/缩进，编号规则表格（编号格式/样式/起始值/分隔符/链接级别/级别重置） |
| **图表** | 图题/表题子标签切换，各自的中英文字体/字号/粗斜体/行距/缩进对齐设置 |
| **目录** | 目录标题样式，4 层级各自的字体/字号/粗斜体/行距/段前段后/缩进/前导符 |
| **页眉页脚** | 页眉内容与字体属性、距离顶部边距；页脚内容与字体属性、页码类型 |
| **初始化** | 文本清理开关（空段落/多余空格/全角半角/制表符/手动换行/手动编号）、样式清理开关（斜体/下划线/字体颜色/高亮/删除线/字符缩放）、对象与结构检测（图片/表格/公式/超链接/书签/交叉引用）、图题表题检测、全局应用开关 |

### 一键排版
- 应用所有标签参数，自动计算排版前后的差异清单
- 差异列表逐项展示，用户确认后可进入对比页面
